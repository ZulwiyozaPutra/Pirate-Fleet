...
class Player {
    ...
    func addPlayerShipsMinesMonsters(_ numberOfMines: Int = 0, numberOfSeamonsters: Int = 0) {
        ...
        for _ in 0..<numberOfMines {
            ...
            var mine = Mine(location: location, penaltyText: "Uh oh spaghetti-o's", guaranteesHit: false)
            while !gridViewController.addMine(mine, playerType: .computer) {
                location = RandomGridLocation()
                mine = Mine(location: location, penaltyText: "Uh oh spaghetti-o's", guaranteesHit: false)
            }
        }
        ...
        for _ in 0..<numberOfSeamonsters {
            var location = RandomGridLocation()
            var seaMonster = SeaMonster(guaranteesHit: true, penaltyText: "You got blasted fool", location: location)
            while !gridViewController.addSeamonster(seaMonster, playerType: .computer) {
                location = RandomGridLocation()
                seaMonster = SeaMonster(guaranteesHit: true, penaltyText: "You got blasted fool", location: location)
            }
        }
    }
}
